...
const managedRuntime = Boolean(process.env.K_SERVICE || process.env.FUNCTION_TARGET)
const secret = process.env.JWT_SECRET || (managedRuntime ? '' : 'a3-local-development-secret-change-me')
if (!secret) throw new Error('JWT_SECRET must be configured in the managed runtime.')
...
const allowedOrigins = new Set([
  'http://localhost:5173',
  'https://yuyang-zhao.web.app',
  'https://yuyang-zhao.firebaseapp.com',
...
app.post('/api/email/send', auth, requireRole('staff'), async (req, res) => {
  ...
  if (!Array.isArray(recipients) || !recipients.length || recipients.length > 50 || !recipients.every((recipient) => validEmail(String(recipient).trim())) || !plainText(subject, 150) || !plainText(body, 5000)) return res.status(400).json({ error: 'Recipients, subject or body are invalid.' })
