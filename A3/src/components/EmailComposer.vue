...
async function send() {
  ...
  try {
    ...
    const result = await apiPost('/email/send', { recipients: props.recipients, subject: subject.value, body: body.value, attachment: encoded })
...
<template>
  <section class="email-card" aria-labelledby="email-title">
    ...
    <p class="muted-copy">The service sends a validated message to the selected member list. Configure Resend or SendGrid keys for delivery.</p>
    <form @submit.prevent="send"><label>Subject<input v-model="subject" type="text" maxlength="150" required></label><label>Message<textarea v-model="body" rows="4" maxlength="5000" required></textarea></label><label class="file-label"><FilePlus2 :size="18" /> Attachment<input type="file" accept=".pdf,.csv,.txt,.png,.jpg" @change="chooseFile"></label><span v-if="attachment" class="field-hint">{{ attachment.name }}</span><button class="primary-button" type="submit" :disabled="sending"><Send :size="18" /> {{ sending ? 'Sending...' : `Send to ${recipients.length} members` }}</button></form>
